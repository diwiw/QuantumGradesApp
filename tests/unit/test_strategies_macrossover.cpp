...
TEST_SUITE("Execution") {
  TEST_CASE("Fixed commission lowers final equity") {
    ...
    qga::strategy::BuyHold bh;
    qga::domain::backtest::Engine eng(1000.0, qga::domain::backtest::ExecParams{ /*fixed*/ 1.0, /*bps*/ 0.0, /*slip*/ 0.0 });
    auto res = eng.run(s, bh);
    CHECK(res.trades_executed_ == 1);
  ...
  TEST_CASE("Slippage worsens buy price and improves sell price") {
    ...
    qga::domain::backtest::Engine eng_no(1000.0, qga::domain::backtest::ExecParams{0.0, 0.0, 0.0});
    qga::domain::backtest::Engine eng_sl(1000.0, qga::domain::backtest::ExecParams{0.0, 0.0, 10.0}); // 10 bps slippage
